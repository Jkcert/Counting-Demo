...
parser.add_argument('--format', type=str,
                    default='open', help='save in the format of cmu or coco or openpose, option: coco/cmu/open')
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if boxes is None or boxes.nelement() == 0:
                    ...
                    # # print(x1, x2, y1, y2)
                    # frame_roi = image[int(boxes[0][1]): int(boxes[0][3]), int(boxes[0][0]): int(boxes[0][2]), :]
                    # pull up init 99 355 229 386 -> init box -> (x1, x2, y1, y2)
                    # pull up box  24 360 211 413 -> box  ->  * ((x1 - 75) * cx, (x2 + 5) * cx, (y1 - 20) * cy, (y2 + 30) * cy)
                    max_area = 0
                    box_idx = 0
                    idx = -1
                    for bbox in boxes:
                        ...
                        if area > max_area:
                            ...
                            # 框出图中RoI区域
                    # global x1, x2, y1, y2
                    # x1, x2, y1, y2 = min(x1, int(boxes[box_idx][1])), max(x2, int(boxes[box_idx][3])), min(y1, int(
                    #     boxes[box_idx][0])), max(y2, int(boxes[box_idx][2]))
                    if self.args.profile:
                        ckpt_time, det_time = getTime(start_time)
                        runtime_profile['dt'].append(det_time)
                    # Pose Estimation
                    ...
                    if self.args.profile:
                        ...
                        runtime_profile['pt'].append(pose_time)
                    hm = hm.cpu()
                    # print(boxes.shape, scores.shape, ids.shape, hm.shape, cropped_boxes.shape)
                    # print(boxes[box_idx].unsqueeze(dim=0).shape, scores[box_idx].shape, ids[box_idx].shape, hm[box_idx].shape, cropped_boxes[box_idx].shape)
                    self.writer.save(boxes[box_idx].unsqueeze(dim=0), scores[box_idx].unsqueeze(dim=0),
                                     ids[box_idx].unsqueeze(dim=0), hm[box_idx].unsqueeze(dim=0),
                                     cropped_boxes[box_idx].unsqueeze(dim=0), orig_img, im_name)
                    # self.writer.save(boxes, scores,
                    #                  ids, hm,
